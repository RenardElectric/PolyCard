...
Each card has a unique method of getting it with a cumulative probability threshold for each rarity.
...
More details on each card and how to get them can be queried using the /polycard info command.

A player can equip up to five cards at a time, and the effects of those cards will be applied to the player.
...
Ten cards of the same type and rarity can be combined to create a card of the same type
but one rarity higher using the /polycard combine command.

Achievements track the player's progress in getting cards.
...
- /polycard info \<cardGroup> \<cardType>
    - Get information about a specific card.


- /polycard equip \[player]
    - Open the equipment manager to equip up to five cards.
...
- /polycard give \<players> \<cardGroup> \<cardType> \[rarityLevel]
    - Give a card to one or more players (admins only).


- /polycard test \<cardGroup> \<cardType> \[cardsNumber]
