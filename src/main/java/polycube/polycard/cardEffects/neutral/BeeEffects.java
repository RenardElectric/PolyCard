    ...
    public void onItemConsumed(ServerPlayer player, ItemStack itemStack) {
        if (itemStack.is(Items.HONEY_BOTTLE)) {
            ...
            conditionsFor(player)
                    .hasRare(() -> {
                        PolyCard.LOGGER.debug("{} has the rare bee card and consumed a honey bottle. Applying speed.", playerName);
                        player.addEffect(new MobEffectInstance(MobEffects.SPEED, SPEED_EFFECT_DURATION, SPEED_EFFECT_AMPLIFIER));
                    })
                    .hasEpic(() -> {
                        PolyCard.LOGGER.debug("{} has the epic bee card and consumed a honey bottle. Applying regeneration.", playerName);
                    ...
                    .hasLegendary(() -> {
                        PolyCard.LOGGER.debug("{} has the legendary bee card and consumed a honey bottle. Applying health boost.", playerName);
